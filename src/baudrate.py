...
#			"4800",
	...
	def NextBaudrate(self, updn):
		...
		sys.stderr.write('\n\n@@@@@@@@@@@@@@@@@@@@@ Baudrate: {}} @@@@@@@@@@@@@@@@@@@@@\n\n'.format(self.BAUDRATES[self.index]))
	...
	def MinicomConfig(self, name=None):
		...
		if name is not None and name:
			try:
				open("/etc/minicom/minirc.{}".format(name), "w").write(config)
...
if __name__ == '__main__':
	...
	def usage():
		...
		print("")
		print("Baudrate v{}".format(baud.VERSION))
		print("Craig Heffner, http://www.devttys0.com")
		print("")
		print("Usage: {} [OPTIONS]".format(sys.argv[0]))
		print("")
		print("\t-p <serial port>       Specify the serial port to use [/dev/ttyUSB0]")
		print("\t-t <seconds>           Set the timeout period used when switching baudrates in auto detect mode [{}]".format(baud.READ_TIMEOUT))
		print("\t-c <num>               Set the minimum ASCII character threshold used during auto detect mode [{}]".format(baud.MIN_CHAR_COUNT))
